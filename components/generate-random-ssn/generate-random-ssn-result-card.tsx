...
export function GenerateRandomSsnResultCard({
  mode,
  values,
  reducedMotion,
  onCopySingleJson,
            ...
            <table className={cn('min-w-full text-sm')}>
              <thead>
                <tr className={cn('border-b border-border/70 bg-muted/35')}>
                  <th className={cn('px-3 py-2 text-left font-medium')}>SSN</th>
                  <th className={cn('px-3 py-2 text-left font-medium')}>Area</th>
                  <th className={cn('px-3 py-2 text-left font-medium')}>Group</th>
                  <th className={cn('px-3 py-2 text-left font-medium')}>Serial</th>
                </tr>
              </thead>
              <tbody>
                {values.map(value => (
                  <tr
                    key={`${value.ssn}-${value.serial}`}
                    className={cn('border-b border-border/50 last:border-b-0')}>
                    <td className={cn('px-3 py-2 font-mono')}>{value.ssn}</td>
                    <td className={cn('px-3 py-2 font-mono')}>{value.area}</td>
                    <td className={cn('px-3 py-2 font-mono')}>{value.group}</td>
                    <td className={cn('px-3 py-2 font-mono')}>{value.serial}</td>
                  </tr>
                ))}
              </tbody>
            </table>
