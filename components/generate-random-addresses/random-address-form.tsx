...
import { Loader2, MapPinned, RefreshCw, WandSparkles } from 'lucide-react';
...
import { GenerateRandomAddressesResultCard } from './generate-random-addresses-result-card';

type RandomAddressFormProps = {
...
export function RandomAddressForm({
  reducedMotion,
  panelRef,
  resultsRef,
  mode,
      ...
      <div className={cn('mx-auto max-w-3xl space-y-8 sm:space-y-10')}>
          ...
          <CardHeader
            ...
            <div className={cn('flex items-start gap-3.5 sm:items-center')}>
              <span
                ...
                <MapPinned className={cn('size-5')} aria-hidden />
              </span>
              <div className={cn('min-w-0 space-y-1 text-left')}>
                ...
                <CardDescription className={cn('text-sm leading-relaxed')}>
                  Choose generation mode and create fictional addresses.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
        ...
        <motion.div
          ...
          <AnimatePresence initial={false}>
            {values.length > 0 ? (
              <GenerateRandomAddressesResultCard
                mode={mode}
                values={values}
