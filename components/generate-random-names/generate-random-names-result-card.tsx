...
export type RandomNamesCopyState = 'idle' | 'copied' | 'failed';

function resultTitle(count: number, nameType: RandomNameType): string {
  if (count === 0) {
    return 'No names';
  }
  const n = count === 1 ? '1' : String(count);
  if (nameType === 'firstName') {
    return `${n} ${count === 1 ? 'first name' : 'first names'}`;
  }
  if (nameType === 'lastName') {
    return `${n} ${count === 1 ? 'last name' : 'last names'}`;
  }
  return `${n} ${count === 1 ? 'full name' : 'full names'}`;
}

type GenerateRandomNamesResultCardProps = {
  names: string[];
  nameType: RandomNameType;
  copyState: RandomNamesCopyState;
  onCopy: () => void;
  className?: string;
};

export function GenerateRandomNamesResultCard({
  names,
  nameType,
  copyState,
  onCopy,
  className,
}: GenerateRandomNamesResultCardProps) {
  return (
    <Card
      className={cn(
      ...
      <CardHeader className='flex flex-col gap-4 border-b border-border/80 px-4 pb-5 pt-6 sm:flex-row sm:items-start sm:justify-between sm:gap-6 sm:px-6'>
        <div className='min-w-0 space-y-1'>
          <CardTitle className='text-base font-semibold sm:text-lg'>
            {resultTitle(names.length, nameType)}
          </CardTitle>
