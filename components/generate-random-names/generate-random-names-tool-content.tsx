...
export function GenerateRandomNamesToolContent({
  ...
  const { onUse, onResult, onCopy, onReset } = useAnalytics("random-names");
  ...
  const generate = useCallback(async () => {
    ...
    onUse({ count: n, name_type: nameType });
    clearCopyFeedbackTimer();
    setCopyState("idle");
    setError(null);
    setLoading(true);
    ...
    try {
      ...
      onResult({ count: data.names.length, name_type: data.nameType });
    } catch {
      setError("Something went wrong. Check your connection and try again.");
    } finally {
      setLoading(false);
    }
  }, [limit, nameType, clearCopyFeedbackTimer, onUse, onResult]);
  ...
  }, [names, clearCopyFeedbackTimer, onCopy]);
