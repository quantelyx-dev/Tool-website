...
export function DailyCompoundExportCard({
  ...
  const handlePdf = useCallback(async () => {
    ...
    setPdfBusy(true);
    try {
      const { exportDailyCompoundExplanationPdf } = await import(
        '@/lib/daily-compound-interest/export-pdf'
      );
    ...
    <Card className={cn('shadow-xs', className)}>
      ...
      <CardContent
        ...
        <p id={EXPORT_FOOTNOTE_ID} className='text-xs text-muted-foreground'>
          Excel and CSV carry full tabular snapshots. PDF today is an explanatory
          companion (methodology plus your headline numbers); TOC, richer tables,
          and chart inset will arrive in a future update.
        </p>
      </CardContent>
    </Card>
  );
