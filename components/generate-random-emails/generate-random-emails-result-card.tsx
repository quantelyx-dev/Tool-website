...
export type RandomEmailsCopyState = 'idle' | 'copied' | 'failed';

type GenerateRandomEmailsResultCardProps = {
  emails: string[];
  copyState: RandomEmailsCopyState;
  onCopy: () => void;
  className?: string;
};

export function GenerateRandomEmailsResultCard({
  emails,
  copyState,
  onCopy,
  className,
}: GenerateRandomEmailsResultCardProps) {
    ...
    <Card
      ...
      <CardContent className='px-4 pb-6 pt-5 sm:px-6 sm:pt-6'>
        <div
          ...
          role='list'
          aria-label='Generated emails'>
          <ul className='flex flex-wrap gap-2 sm:gap-2.5'>
            {emails.map((email, i) => (
              <li key={`${i}-${email}`} role='listitem'>
                <span
                  className={cn(
