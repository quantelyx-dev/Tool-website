...
import { Check, Copy, Download, Network, X } from 'lucide-react';

import type { GeneratedIp } from '@/lib/generate-random-ip-addresses/generate';
...
function versionLabel(version: GeneratedIp['version']): string {
  return version === 'v4' ? 'IPv4' : 'IPv6';
}

export function GenerateRandomIpAddressesResultCard({
  mode,
  values,
  reducedMotion,
  onCopySingleValue,
  ...
  const versionText = values[0] ? versionLabel(values[0].version) : '';
          ...
          <div className={cn('flex items-start gap-3.5 sm:items-center')}>
            ...
            <div className={cn('min-w-0 space-y-1 text-left')}>
              <CardTitle className={cn('text-base font-semibold leading-snug')}>
                {single
                  ? `Generated ${versionText} address`
                  : `Generated ${versionText} addresses (${values.length})`}
              </CardTitle>
              <CardDescription className={cn('text-sm leading-relaxed')}>
                {single
                  ? 'Copy this IP address to your clipboard.'
                  : 'Export the generated IP addresses as CSV.'}
              </CardDescription>
            </div>
          </div>
        </CardHeader>
              ...
              <tbody>
                {values.map((value, index) => (
                  <tr
                    ...
                    <td className={cn('px-3 py-2 font-mono')}>{value.ip}</td>
                  </tr>
                ))}
              </tbody>
            </table>
